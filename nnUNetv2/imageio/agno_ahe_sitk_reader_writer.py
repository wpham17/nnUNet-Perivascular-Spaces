...
## WP: IGNORE THIS - use agnostik reader write
## and normalisation T1wPVSNormalisation preprocessor
...
class AgnoAHESITKIO(BaseReaderWriter):
    ...
    def read_images(self, image_fnames: Union[List[str], Tuple[str, ...]]) -> Tuple[np.ndarray, dict]:
        ...
        for f in image_fnames:
            ...
            #agnostic_spacing = tuple([1 for i in orig_spacing])
            #spacings.append(agnostic_spacing)
            #spacings.append(tuple(np.ones(np.shape(orig_spacing))))
            spacings.append(itk_image.GetSpacing())
            origins.append(itk_image.GetOrigin())
            directions.append(itk_image.GetDirection())
            npy_image = sitk.GetArrayFromImage(itk_image)
            #AHE
            p1, p2 = np.percentile(npy_image, (1,99))
            npy_image = rescale_intensity(npy_image, in_range=(p1, p2))
            npy_image = equalize_adapthist(npy_image)
            if npy_image.ndim == 2:
                # 2d
